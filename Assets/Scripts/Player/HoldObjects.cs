...
public class HoldObjects : MonoBehaviour
{
        ...
        if (!PauseMenu.GameIsPaused || canHold)
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                else
                {
                    ...
                    GameObject.Find(name).GetComponent<Memes>().StartCoroutine("NotHolding"); //no funciono para detener el reverb
    ...
    void PickUpObject(GameObject pickObject)
    {
        if (pickObject.GetComponent<Rigidbody>())
        {
            ...
            string name; name = heldObject.transform.name;
            GameObject.Find(name).GetComponent<Memes>().isHolded = true;
    ...
    {        
        ...
        string name; name = heldObject.transform.name;
        GameObject.Find(name).GetComponent<Memes>().isHolded = false;
    ...
    {        
        ...
        if (heldObject != null && heldObject == orbe) //si tenes un objeto agarrado y si es el mismo que entro al trigger
        {
            string name; name = heldObject.transform.name;
            GameObject.Find(name).GetComponent<Memes>().isHolded = false;
            GameObject.Find(name).GetComponent<Memes>().StartCoroutine("NotHolding");
